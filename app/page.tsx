...
export default function Home() {
  ...
  }, [depenses, salaires, mesPositions, isMounted]);
  ...
  const getSalaireForMois = (mIso: string) => {
      ...
      if (pastMois.length > 0) return salaires[pastMois[pastMois.length - 1]];
      return 0;
  };
  ...
  return (
    <div style={{ background: "#050505", color: "#fff", minHeight: "100vh", fontFamily: "-apple-system, sans-serif", paddingBottom: "100px" }}>
      ...
      <div style={{ display: "flex", background: "#0a0a0a", borderBottom: "1px solid #111", position: "sticky", top: "65px", zIndex: 99, overflowX: "auto" }}>
        {[ { id: "pea", label: "📈 PEA" }, { id: "budget", label: "💰 BUDGET" }, { id: "bilan", label: "📊 BILAN" }, { id: "analyse", label: "🎲 ANALYSE" } ].map((tab) => (
          <button key={tab.id} style={{ flex: 1, minWidth: "80px", padding: "15px 5px", background: activeTab === tab.id ? "#111" : "none", color: activeTab === tab.id ? "#fff" : "#555", border: "none", borderBottom: activeTab === tab.id ? "2px solid #e8a45d" : "none", fontWeight: "bold", cursor: "pointer", fontSize: "11px", whiteSpace: "nowrap", transition: "0.2s" }} onClick={() => setActiveTab(tab.id)}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: "20px", maxWidth: "600px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        {activeTab === "bilan" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <div style={{ background: "linear-gradient(145deg, #161619, #0a0a0a)", padding: "20px", borderRadius: "15px", border: "1px solid #1e1e28", textAlign: "center" }}>
                    <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", marginBottom: "5px" }}>Revenus Annuels</div>
                    <div style={{ fontSize: "20px", fontWeight: "bold", color: "#3dd68c" }}>{f0(totalAnnuelSal)} €</div>
                </div>
                <div style={{ background: "linear-gradient(145deg, #161619, #0a0a0a)", padding: "20px", borderRadius: "15px", border: "1px solid #1e1e28", textAlign: "center" }}>
                    <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", marginBottom: "5px" }}>Dépenses Annuelles</div>
                    <div style={{ fontSize: "20px", fontWeight: "bold", color: "#f05656" }}>{f0(totalAnnuelDep)} €</div>
                </div>
                <div style={{ background: "linear-gradient(145deg, #161619, #0a0a0a)", padding: "20px", borderRadius: "15px", border: "1px solid #1e1e28", textAlign: "center" }}>
                    <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", marginBottom: "5px" }}>Épargne & Investi</div>
                    <div style={{ fontSize: "20px", fontWeight: "bold", color: "#3b82f6" }}>{f0(totalAnnuelEpa)} €</div>
                </div>
                <div style={{ background: "linear-gradient(145deg, #161619, #0a0a0a)", padding: "20px", borderRadius: "15px", border: "1px solid #1e1e28", textAlign: "center" }}>
                    <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", marginBottom: "5px" }}>Taux d'Épargne</div>
                    <div style={{ fontSize: "20px", fontWeight: "bold", color: "#e8a45d" }}>{fp(tauxEpargneAnnuel)}</div>
                </div>
            </div>
          ...
          <>
            ...
            <div style={{ background: "#0a0a0a", padding: "20px", borderRadius: "15px", border: "1px solid #111" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", fontSize: "10px", color: "#888" }}><span style={{color: "#e8a45d"}}>● Valeur</span><span style={{color: "#444"}}>● Dépôts</span></div>
              </div>
            ...
            <div style={{ background: "#0a0a0a", padding: "20px", borderRadius: "15px", border: "1px solid #111", overflowX: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
                  <div style={{ fontSize: "12px", color: "#555", fontWeight: "bold" }}>DÉTAIL DE TES ETF</div>
                  <button onClick={() => setShowAddETF(!showAddETF)} style={{ background: "#e8a45d", color: "#000", border: "none", borderRadius: "5px", padding: "5px 10px", fontSize: "10px", fontWeight: "bold", cursor: "pointer" }}>{showAddETF ? "Fermer" : "+ Acheter"}</button>
              </div>

              {showAddETF && (
                  <div style={{ background: "#111", padding: "15px", borderRadius: "10px", marginBottom: "15px", border: "1px dashed #333" }}>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                          <input type="text" placeholder="Ticker (CW8.PA)" value={etfTicker} onChange={(e) => setEtfTicker(e.target.value)} style={{ padding: "10px", borderRadius: "5px", background: "#050505", border: "1px solid #222", color: "#fff", fontSize: "12px" }} />
                          <input type="text" placeholder="Nom (Opt)" value={etfNom} onChange={(e) => setEtfNom(e.target.value)} style={{ padding: "10px", borderRadius: "5px", background: "#050505", border: "1px solid #222", color: "#fff", fontSize: "12px" }} />
                          <input type="number" placeholder="Qté" value={etfQty} onChange={(e) => setEtfQty(e.target.value)} style={{ padding: "10px", borderRadius: "5px", background: "#050505", border: "1px solid #222", color: "#fff", fontSize: "12px" }} />
                          <input type="number" placeholder="PRU €" value={etfPru} onChange={(e) => setEtfPru(e.target.value)} style={{ padding: "10px", borderRadius: "5px", background: "#050505", border: "1px solid #222", color: "#fff", fontSize: "12px" }} />
                      </div>
                      <button onClick={acheterETF} style={{ width: "100%", padding: "10px", background: "#3dd68c", color: "#000", border: "none", borderRadius: "5px", fontWeight: "bold", cursor: "pointer" }}>Confirmer l'achat</button>
                  </div>
              )}

              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead><tr><th style={{ textAlign: "left", paddingBottom: "10px", color: "#555", fontWeight: "normal" }}>ETF</th><th style={{ textAlign: "left", paddingBottom: "10px", color: "#555", fontWeight: "normal" }}>Qté</th><th style={{ textAlign: "left", paddingBottom: "10px", color: "#555", fontWeight: "normal" }}>Cours</th><th style={{ textAlign: "left", paddingBottom: "10px", color: "#555", fontWeight: "normal" }}>P/L</th></tr></thead>
                <tbody>
                  {positions.map((p: any) => (
                    <tr key={p.ticker || p.nom} style={{ borderBottom: "1px solid #111" }}>
                      <td style={{ padding: "12px 0" }}>{p.nom} <div style={{fontSize: "9px", color: "#555"}}>{p.ticker} (PRU: {f2(p.pru)}€)</div></td>
            ...
            <div style={{ background: "#0a0a0a", padding: "20px", borderRadius: "15px", border: "1px solid #111" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}><span style={{ color: "#aaa" }}>Plus-value AVANT impôts</span><span style={{ color: totalPL >= 0 ? "#fff" : "#f05656", fontWeight: "bold" }}>{f2(totalPL)} €</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}><span style={{ color: "#aaa" }}>Prélèvements Sociaux (17.2%)</span><span style={{ color: "#f05656" }}>-{f2(fiscalite5ans)} €</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: "10px", paddingTop: "10px", borderTop: "1px dashed #333", fontWeight: "bold" }}><span style={{ color: "#3dd68c" }}>BÉNÉFICE NET APRÈS IMPÔTS</span><span style={{ color: "#3dd68c" }}>{f2(gainsNets5ans)} €</span></div>
            </div>
          </>
        )}
        ...
        {activeTab === "budget" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" }}>
              <div style={{ background: "#0a0a0a", padding: "15px", borderRadius: "15px", border: "1px solid #111" }}>
                <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", marginBottom: "5px" }}>Revenus ce mois</div>
                <input type="number" value={salaires[moisActuel] !== undefined ? salaires[moisActuel] : ""} placeholder={getSalaireForMois(moisActuel).toString()} onChange={(e) => updateSalaire(e.target.value)} style={{ background: "none", border: "none", color: "#3dd68c", fontSize: "20px", fontWeight: "bold", width: "100%", padding: 0 }} />
              </div>
              <div style={{ background: "#0a0a0a", padding: "15px", borderRadius: "15px", border: "1px solid #111" }}>
            ...
            <div style={{ background: "#0a0a0a", padding: "20px", borderRadius: "15px", border: "1px solid #111" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontSize: "13px" }}>🏦 Versé PEA</div><div style={{ fontWeight: "bold", color: "#a78bfa" }}>{f2(totalCagnottePEA)} €</div></div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontSize: "13px" }}>🛡️ Épargne Sécurité</div><div style={{ fontWeight: "bold", color: "#3b82f6" }}>{f2(totalCagnotteSecu)} €</div></div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontSize: "13px" }}>✈️ Voyage</div><div style={{ fontWeight: "bold", color: "#e8a45d" }}>{f2(totalCagnotteVoyage)} €</div></div>
                </div>
            </div>
            ...
            {depensesDuMois.length > 0 && (
                <div style={{ background: "#0a0a0a", padding: "20px", borderRadius: "15px", border: "1px solid #111" }}>
                    ...
                    <div style={{ display: "flex", gap: "15px", marginTop: "15px", flexWrap: "wrap" }}>
                      {Object.entries(budgetCats).map(([cat, val]) => (
                        <div key={cat} style={{ fontSize: "11px", color: "#aaa", display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "8px", height: "8px", borderRadius: "50%", background: catColors[cat] || "#888" }} />{cat} ({f2(val as number)} €)</div>
                      ))}
                    </div>
                </div>
            )}
        ...
        {activeTab === "analyse" && (
            <div style={{ background: "#0a0a0a", padding: "20px", borderRadius: "15px", border: "1px solid #111" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}><span style={{ fontSize: "11px", color: "#aaa" }}>Investissement mensuel :</span><strong style={{color: "#fff", fontSize: "14px"}}>{simMensuel} €</strong></div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px", fontSize: "10px", fontWeight: "bold" }}><span style={{color: "#3dd68c"}}>● Top 10% ({f0(p90[15] || 0)}€)</span><span style={{color: "#e8a45d"}}>● Médiane ({f0(p50[15] || 0)}€)</span><span style={{color: "#f05656"}}>● Pire 10% ({f0(p10[15] || 0)}€)</span></div>
      ...
      {(activeTab === "budget" || activeTab === "bilan") && (
          <button onClick={() => setShowFabModal(true)} style={{ position: "fixed", bottom: "30px", right: "30px", width: "60px", height: "60px", borderRadius: "30px", background: "#e8a45d", color: "#000", fontSize: "30px", border: "none", boxShadow: "0 4px 15px rgba(232,164,93,0.4)", cursor: "pointer", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
      )}
